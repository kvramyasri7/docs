...
<Callout warning>
  ...
  **By default, owners have full permissions on their records!**

  Unless [field-level authorization](#field-level-authorization-rules) is enabled, although an owner field is automatically generated, the 'owner' of a record has full update/delete permissions, which can include updating the owner to a nonexistent or different user. If this is not desirable behavior, field-level authorization may be configured on the owner field itself.
